...
| # | Case | 源脚本位置 | 核心参数 | 比对库 |
|---|------|-----------|---------|--------|
| 1 | 高斯模糊 + 透明度 | `05_写代码做视频的工程/...图层运动工程调整与待办脚本.md` 第49行 | Opacity 100%→35%, Blur 0→18px, 1.5s线性 | CSS filter · Canvas 2D · GSAP · PixiJS |
| 2 | 弹性弹出 | 同上第47行 | Scale 0%→100%, spring(damping:15, mass:0.6) | GSAP · Motion · Anime.js · CSS keyframes |
| 3 | ClipPath 擦除 | 同上第49行 | inset(0% 100% 0% 0%) → inset(0% 0% 0% 0%), 1s | CSS clip-path · GSAP · Motion · Canvas 2D |
| 4 | 局部放大镜 | 同上第49-50行 | circle(120px), 内部scale(2.5), ±2px抖动 | CSS clip-path · Canvas 2D · PixiJS |
| 5 | 错层入场 | 同上第49行; Few-Shot第37行 | 3条卡片staggered, 间隔0.8-1.2s | GSAP stagger · Motion staggerChildren · CSS delay · Anime.js |

### P1 级（2 个）

| # | Case | 源脚本位置 | 核心参数 | 比对库 |
|---|------|-----------|---------|--------|
| 6 | 打字机 + 光标 | Few-Shot 第35行 | 逐字出现1-1.5s, 光标0.3s闪烁 | JS interval · GSAP · CSS steps · Canvas |
| 7 | 阴影发光 | 动效脚本第50行 | drop-shadow 0→30px, spring damping:10 | CSS filter · Canvas · PixiJS · GSAP |

### P2 级（3 个）

| # | Case | 源脚本位置 | 核心参数 | 比对库 |
|---|------|-----------|---------|--------|
| 8 | 贝塞尔路径 | 拉片 prompt 第30行 | cubic-bezier(0.25,0.1,0.25,1.0) | CSS bezier · GSAP motionPath · Motion · Anime.js |
| 9 | 数字滚动 | 逐镜头实现表 S14 | 12-18帧滚动到位 | GSAP · Anime.js · JS rAF · CSS @property |
| 10 | SVG 路径描边 | 逐镜头实现表 S08 | 多路径时序点亮 | CSS stroke-dasharray · GSAP drawSVG · Anime.js |
